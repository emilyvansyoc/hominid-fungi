...
host <- hom
#### 11/18; drop G beringei for clarity
#host <- drop.tip(host, tip = "Gorilla_beringei")
...
geom_tree(layout = "slanted", size = 2, position = position_nudge(x = -8, y = 0.8)) +
  ...
  ### ADD FUNGI PHYLOPICS ####
geom_phylopic(data = homgor %>% filter(rangecol %in% c("rangeearly", "range", "rangelate")), aes(uuid = ypic, x = -mean, y = 4, fill = forcolor), size = 0.3, color = "black") +
  geom_phylopic(data = pangor %>% filter(rangecol %in% c("rangeearly", "range", "rangelate")), aes(uuid = ypic, x = -mean, y = 1.5, fill = forcolor), size = 0.3, color = "black") +
...
# G gorilla
add_phylopic(uuid = "142e0571-3b5f-443d-a887-b572a224ea22", x = 0.2, y = 2, ysize = 0.6, fill = "#31688EFF") +
  # human
  add_phylopic(uuid = "036b96de-4bca-408e-adb6-2154fcd724ef", x = -1.8, y = 3, ysize = 0.9, fill = "#35B779FF") +
  # chimp
  add_phylopic(uuid = "7133ab33-cc79-4d7c-9656-48717359abb4", x = -2.8, y = 4, ysize = 0.6, fill = "#FDE725FF") +
