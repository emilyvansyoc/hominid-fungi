...
## load phyloseq
load("private/hominidphylo_ITS.RData")
# remove Mangabey 
psfix <- psf %>% tax_fix() %>% 
...
allfa <- readDNAStringSet("data/all.fasta") ### not uploaded to Github (a little bit big)

# get metadata for hominid name
meta <- readxl::read_xlsx("private/EVS_MetadataITS.xlsx") %>% 
...
outdir <- "data/genus_haplotypes/" #### not run directly in Github; this creates a bunch of files some of which are a little large 
...
setwd("analyses/cophylogeny-scripts/")
